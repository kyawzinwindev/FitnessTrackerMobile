...
class HomeFragment : Fragment() {
    ...
    override fun onResume() {
        ...
        binding.welcomeText.text =
            "Hello, ${sessionManager.getFirstName() ?: ""} ${sessionManager.getLastName() ?: ""}"
    ...
    private fun fetchActivityHistory(userId: Int) {
        val url =
            "http://10.0.2.2:81/FitnessTrackerAPI/controllers/ActivitiesController.php?user_id=$userId"
    ...
    private fun setupChart(activities: JSONArray) {
        val activityTypes =
            arrayOf("Running", "Cycling", "Swimming", "Hiking", "Yoga", "WeightLifting")
        ...
        for (i in 0 until activities.length()) {
            ...
            if (caloriesPerActivity.containsKey(type)) {
                caloriesPerActivity[type] =
                    caloriesPerActivity.getValue(type) + calories
            }
        }
    ...
    private fun getLastLocation() {
        if (ActivityCompat.checkSelfPermission(
            ...
            fusedLocationClient.lastLocation.addOnSuccessListener { location ->
                location?.let {
                    binding.latitudeText.text = "Latitude: ${it.latitude}"
                    binding.longitudeText.text = "Longitude: ${it.longitude}"
                }
            }
        }
    }
